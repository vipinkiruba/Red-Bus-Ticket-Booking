...
map<char, Bus> ACSleep[7];              // bus database
...
void cancel_seat(int s) {
	switch (s) {
		...
		if (pr->second.Booker_name == log_Cust.name) {          // check login name and booker name matches
			if (confirm_cseat(1)) {                             // confirm cancellation
...
void check_availablity(string pname , char g , int r , char c , int s) {
	...
	case 1: {                                                                // AC Sleeper
		...
		if (r == 1 || r == 2 || r == 4 || r == 5) {                           // AC Sleeper Double Seats
			auto pr = ACSleep[r].find(c);
			...
			if (pr->second.Passenger_name == "") {
				...
				if (r == 1 || r==4) {
					...
					if (pseat->second.gender == pr->second.gender || pseat->second.Booker_name == pr->second.Booker_name || pseat->second.gender == 'o') {
						...
						if (confirm_seat(1)) {                          // get confirmation
							
							cout << "booked" << endl;
							pr->second.amount = (pr->second.amount) + 700;
							ACSleep_count--;                                //if booked na amount will be added
...
void book_seat(int s) {
	...
	case 1: {
		...
		for (int i = 1; i <= 6; i++) {  // looping in array of (first NUMBER is row)
		...
		check_availablity(pname, g, r, c ,s);             //checking is done

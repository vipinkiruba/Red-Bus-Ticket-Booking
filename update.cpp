...
Bus ACSleeper[6][2];                   // bus database
...
bool confirm_cseat(int s, int r = -1) {
	...
	case 1: {
		...
		if (r == -1) {
			cout << "cancellation charge per seat will be : 50%" << endl;
		}
...
void cancel_seat(int s) {
	...
	case 1: {                                           // enter position to be canceled
		...
		cout << "seats booked by "<<log_Cust.name<<" :" << endl;
		for (auto itr = log_Cust.booking_id.begin(); itr != log_Cust.booking_id.end(); itr++) {
			cout <<"with booking id : "<< * itr << " "<<endl;
			for (int i = 0; i <= 5; i++) {
				for (int j = 0; j <= 1; j++) {
					if (ACSleeper[i][j].booking_id == *itr) {
						cout << i + 1 << ((j == 0) ? 'A' : 'B') << " ";
					}
				}
			}cout << endl;
		}
		cout << "to cancel by booking id : press 1" << endl;
		cout << "to cancel a particular seat : press 2"<<endl;
		int q; cin >> q;
		switch (q) {
			case 1:
			{
				...
				if (confirm_cseat(1)) {
					for (int r = 0; r <= 5; r++) {
						for (int c = 0; c <= 1; c++) {
							if (ACSleeper[r][c].booking_id == bi) {
								...
								if (ACSleeper[r][c].Booker_name == log_Cust.name) {          // check login name and booker name matches
									...
									ACSleeper[r][c].booking_id = -1;
									//cout << "cancelled" << endl;
									if (r == 3 || r == 4) {
										ACSleeper[r][c].amount = (ACSleeper[r][c].amount) - 350;     //keep the balance amount
										cd_obj.amount = 350;
										cd_obj.name = log_Cust.name;
										cd_obj.bid = bi;
										cdata.push_back(cd_obj);
									}
									if (r == 0 || r == 1 || r == 5) {
				...
				if (ACSleeper[r][c].Booker_name == log_Cust.name) {          // check login name and booker name matches
					if (confirm_cseat(1, r)) {                             // confirm cancellation
						...
						if (r == 3 || r == 4) {
							...
							cd_obj.amount = 350;
							cd_obj.name = log_Cust.name;
							cd_obj.bid = ACSleeper[r][c].booking_id;
							cdata.push_back(cd_obj);
						}
						if (r == 0 || r == 1 || r == 5) {
							ACSleeper[r][c].amount = (ACSleeper[r][c].amount) - 400;
		...
		if (r == 2 || r == 5) {                                              // side single seats
			...
			if (ACSleeper[r][c].Passenger_name == "") {
				if (confirm_seat(1, r)) {
					...
					log_Cust.booking_id.insert(B_Id);                     //inserting booking id in cust set
					ACSleeper[r][c].booking_id = B_Id;                    // insert booking id in seat
				...
				if (r == 0 || r == 3) {
					...
					if (ACSleeper[r + 1][c].gender == ACSleeper[r][c].gender || ACSleeper[r + 1][c].Booker_name == ACSleeper[r][c].Booker_name || ACSleeper[r + 1][c].gender == 'o') {
						...
						if (confirm_seat(1, r)) {                          // get confirmation
							...
							log_Cust.booking_id.insert(B_Id);
							ACSleeper[r][c].booking_id = B_Id;
...
int dashboard() {
	...
	while (1) {
		...
		case 1: {
			...
			cin >> n;
			if (n <= ACSleep_count) {
				B_Id++;                                    //booking id incremented for unique id
				for (int i = 0; i < n; i++) {
					book_seat(1);
				}

				auto itr = Customer_DB.find(log_Cust.username);
				itr->second = log_Cust;
...
void total_amount_details() {
	cout << "canceled details" << endl;
	for (int i = 0; i < cdata.size(); i++) {
		cout << cdata[i].bid << " " << cdata[i].name << " " << cdata[i].amount << endl;;
	}
	cout << endl;
}
...
//press 11 for testing
void display_cust() {
	for (auto elem = Customer_DB.cbegin(); elem != Customer_DB.cend(); ++elem)
	{
		cout << elem->second.name;
		for (auto itr = elem->second.booking_id.begin(); itr != elem->second.booking_id.end(); itr++) {
			cout << *itr << " ";
		}cout << endl;
	}
} //press 11
